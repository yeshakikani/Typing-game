    ...
    <div className="wrong-msg" role="alert">
      ...
      <div className="wm-body">
        ...
        <div className="wm-detail">
          <span style={{ opacity: .7 }}>You typed: </span>
          <span>{typed.length > 60 ? typed.slice(0, 60) + '…' : typed}</span>
          <br />
          <span style={{ opacity: .7 }}>Expected: </span>
          <span>{short}</span>
        </div>
      </div>
    </div>
...
function App() {
  ...
  const loadNext = useCallback((q, l) => {
    let arr = q.length < 3 ? [...q, ...shuf(src(l))] : q;
  ...
  const addTime = useCallback((sec) => {
    ...
    timerR.current = setInterval(() => {
      setTLeft(t => { if (t <= 0.1) { clearInterval(timerR.current); setScreen('over'); return 0; } return +(t - 0.1).toFixed(1); });
    }, 100);
  }, []);
  ...
  const handleCorrect = useCallback((typedLen) => {
    ...
    if (ns % cfg.sg === 0) {
      ...
      setShowBonus(true); clearTimeout(bonusR.current);
        ...
        <div className="screen-start">
          ...
          <div className="level-grid">
            {['easy', 'medium', 'hard'].map(l => {
              const c = LCFG[l]; const sel = selLvl === l;
          ...
          <button className={`btn-go${selLvl ? ' rdy' : ''}`} onClick={startGame}>START GAME</button>
          <div className="how">
            <h3>How to play</h3>
            <ul>
              <li>Easy: type 3-letter words → press Space to submit</li>
              <li>Medium: type full sentences → press Enter to submit</li>
              <li>Hard: type full paragraphs → press Enter when done</li>
      ...
      <div className={wrapCls}>
        <div className="screen-over">
          ...
          <div className="btns">
            ...
            <button className="br sec" onClick={() => { setScreen('start'); setSelLvl(null) }}>CHANGE LEVEL</button>
          </div>
        </div>
      </div>
    );
      ...
      <div className="screen-game">
        ...
        <div className="top-bar">
          ...
          <div className="tbar-wrap">
            <div className="tbar-fill" style={{ width: `${timerPct}%`, background: timerColor }}></div>
          </div>
          <div className="sbox">
        ...
        <div className="meta-row">
          ...
          <div className="streak-row">
            {Array.from({ length: sg }, (_, i) => (
              <div key={i} className={`pip${(streak % sg) > i ? ' on' : ''}`}></div>
            ))}
            <span className="stxt">{streak > 0 ? `${streak} streak` : 'streak'}</span>
          </div>
        </div>
        {wrongInfo && <WrongMsg typed={wrongInfo.typed} expected={wrongInfo.expected} level={lvl} />}
        {showBonus && <div className="bonus-bar show">{`+${cfg.bs}s — ${bonusMsg}`}</div>}
        <div className="word-box">
          <div className={dispCls}>{renderChars(current, inp)}</div>
          <div className="type-hint">{isEasy ? 'type the word · space to submit' : isMed ? 'type the sentence · enter to submit' : 'type the paragraph · enter when done'}</div>
        </div>
